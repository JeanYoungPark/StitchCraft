...
export const knittingTipsData: KnittingTip[] = [
  ...
  // 추가 팁들 - 나중에 더 쉽게 추가할 수 있도록
  {
    ...
    content: '뜨개질 속도는 정확성이 먼저입니다. 처음에는 천천히 정확하게 뜨는 것이 중요해요. 익숙해지면 자연스럽게 속도가 빨라집니다. 손목과 손가락의 움직임을 최소화하고 리듬감 있게 뜨는 것이 포인트예요.',
    difficulty: 'easy'
  },
  {
    id: '14',
    category: 'technique',
    title: '매직루프 기법 활용하기',
    content: '원형바늘로 작은 둘레의 원형 뜨개질을 할 때 매직루프 기법을 사용하면 편리합니다. 바늘의 케이블을 이용해 코를 반으로 나누어 뜨는 방법으로, 양말이나 모자 뜨기에 유용해요.',
    difficulty: 'hard'
  },
  {
    id: '15',
    category: 'problem',
    title: '실이 엉켰을 때 해결법',
    content: '실이 엉켰을 때는 급하게 풀려고 하지 마세요. 엉킨 부분을 잘 관찰하고 천천히 하나씩 풀어나가면 됩니다. 바늘이나 핀을 이용해서 매듭을 느슨하게 만든 후 풀면 더 쉬워요.',
    difficulty: 'easy'
  }
];

// 카테고리별 필터링 함수들
export const getTipsByCategory = (category: KnittingTip['category'] | 'all') => {
  if (category === 'all') return knittingTipsData;
  return knittingTipsData.filter(tip => tip.category === category);
};

// 난이도별 필터링 함수
export const getTipsByDifficulty = (difficulty: KnittingTip['difficulty'] | 'all') => {
...
export const searchTips = (query: string) => {
  ...
  return knittingTipsData.filter(tip => 
